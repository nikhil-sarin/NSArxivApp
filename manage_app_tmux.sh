...
start_app() {
    ...
    tmux new-session -d -s "$SESSION_NAME" \
        "bash -lc 'cd \"$SCRIPT_DIR\" && if [ -f .env ]; then set -a && source ./.env && set +a; fi && source \"$CONDA_SH\" && conda activate \"$ENV_NAME\" && exec streamlit run main.py --server.headless true --server.address 127.0.0.1 --server.port \"$PORT\" --browser.gatherUsageStats false --server.enableStaticServing true --server.fileWatcherType none'"
